...
class PossessionCalculator:
    ...
    def __init__(self, distance_threshold: float = 60.0, fps: float = 25.0):
        ...
        # USUNIĘTO: self._cached_percentage
    ...
    def update(
        ...
        """Aktualizuje posiadanie dla danej klatki."""
        # USUNIĘTO: self._cached_percentage = None

        if ball_position is None or len(players_bboxes) == 0:
            if self.last_possessing_team is not None:
                self. possession_per_frame[frame_idx] = self.last_possessing_team
        ...
        if min_distance < self.distance_threshold:
            ...
            self.possession_per_frame[frame_idx] = team_id
